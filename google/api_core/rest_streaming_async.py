...
class ResponseAsyncIterator:
    ...
    def __init__(
        ...
        self._response_itr = self._response.content.iter_any()
    ...
    async def __anext__(self):
        if not self._data:
            self._data = await self._response.read()
            self._process_chunk(self._data.decode('utf-8'))
        
        if not self._ready_objs:
            if self._level > 0:
