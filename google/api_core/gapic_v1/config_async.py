...
from google.api_core.gapic_v1.config import _timeout_from_retry_config, MethodConfig, _exception_class_for_grpc_status_name, _MILLIS_PER_SECOND


def _retry_from_retry_config(retry_params, retry_codes):
    """Creates an AsyncRetry object given a gapic retry configuration.
    ...
    Returns:
        google.api_core.retry_async.AsyncRetry: The default retry object for the method.
    """
    exception_classes = [
        _exception_class_for_grpc_status_name(code) for code in retry_codes
    ]
    return retry_async.AsyncRetry(
        retry_async.if_exception_type(*exception_classes),
